...
# These are the functions for summarization
from transformers import AutoTokenizer
from rouge_score import rouge_scorer
import tiktoken

class GPTTokenizer:
    def __init__(self, model_name_or_path="gpt-3.5-turbo"):
...
class Rouge_Scorer:
    def __init__(self, model_name_or_path="gpt-3.5-turbo", metrics=['rouge1', 'rougeL']):
        self.model_name_or_path = model_name_or_path
        self.tokenizer = GPTTokenizer(model_name_or_path)
        self.metrics = metrics
        self.rouge_scorer = rouge_scorer.RougeScorer(metrics, tokenizer=self.tokenizer)

    def score(self, target, prediction):
        scores = self.rouge_scorer.score(target=str(target), prediction=str(prediction))
        rouge_scores = [scores[m].fmeasure for m in self.metrics]
        return rouge_scores

# test GPTTokenizer
if __name__ == "__main__":
    tokenizer = GPTTokenizer(model_name_or_path="chatgpt")
